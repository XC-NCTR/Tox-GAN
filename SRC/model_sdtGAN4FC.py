...
class Discriminator(nn.Module):
    ...
    def forward(self, Exp, Stru, Time, Dose):
        # Concatenate condition and real_FC to produce input
...
if not os.path.exists(os.path.join(path, 'model_sdtGAN4FC')):
    os.makedirs(os.path.join(path, 'model_sdtGAN4FC'))
...
for epoch in range(opt.n_epochs):
    ...
    for i, (Stru, Exp, Time, Dose) in enumerate(dataloader):
        ...
        # Sampling noise and and Stru, Time, Dose as generator input
    ...
    if epoch % opt.interval == 0:
        torch.save(generator.state_dict(), os.path.join(path, 'model_sdtGAN4FC', 'generator_{}'.format(epoch)))
        # torch.save(discriminator.state_dict(),
        #            os.path.join(path, 'model_sdtGAN4FC', 'discriminator_{}'.format(opt.model)))
    # end = time.time()
