...
ExpCode = pd.read_csv(os.path.join(path, 'Data', 'ExpCode_repeat.tsv'), index_col=0, sep="\t")
...
model_path = r'../model_sdtGAN4Exp'  # please change it to the folder where your well-trained model save
weights = torch.load(os.path.join(model_path, 'generator_Exp'))
generator = Generator()
if cuda:
    generator.cuda()
...
# df.index = Stru.index
# df.to_csv(os.path.join(path, 'genDM_Exp.tsv'), sep='\t')
np.save(os.path.join(path, 'Results', 'gen_Exp_100.npy'), scaler.inverse_transform(de))
